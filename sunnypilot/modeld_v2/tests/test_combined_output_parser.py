...
def test_legacy_hypothesis_selection_is_preserved(batch, name, in_n, out_n, shape):
  ...
  Parser().parse_mdn(name, outs, in_n, out_n, shape)
...
@pytest.mark.parametrize('name,width', [('plan', n) for n in (0, 989, 991, 4950, 4954, 4956)] +
                         [('lead', n) for n in (0, 101, 103, 143, 145)])
def test_unknown_head_width_is_rejected(name, width):
  with pytest.raises(ValueError, match=f'Invalid {name} output shape'):
