# BluePilot: parser port from sunnypilot#1993, commit 6135084c941d.
# Original author: James Vecellio-Grant (Discountchubbs).
# End BluePilot
import numpy as np
...
def _infer_mhp(slice_size: int, prod_out_shape: int, max_in_n: int = 16, max_out_n: int = 6) -> tuple[int, int]:
  for out_n in range(max_out_n + 1):
    ...
    if slice_size % per == 0:
      ...
      if 1 <= in_n <= max_in_n:
        return in_n, out_n
  # BluePilot: reject unknown layouts rather than guessing and failing in reshape.
  raise ValueError(f"Unsupported model output width {slice_size} for shape size {prod_out_shape}")
  # End BluePilot
