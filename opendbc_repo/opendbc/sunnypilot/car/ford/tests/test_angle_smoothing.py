...
def _smoother(menu: float = 2.0, enabled: bool = True) -> AngleSmoother:
...
class TestStockContract:
  def test_menu_one_is_bit_identical_passthrough(self) -> None:
  ...
  def test_toggle_off_is_bit_identical_passthrough(self) -> None:
  ...
  def test_configure_clamps_menu(self) -> None:
    s = AngleSmoother(dt=DT)
...
class TestEntryGuarantee:
  def test_gain_schedule_rise_is_fast_at_any_strength(self) -> None:
  ...
  def test_release_slows_with_strength(self) -> None:
    outs = {}
    for menu in (1.5, 2.5):
      s = _smoother(menu=menu)
...
class TestTransitionSafety:
  def test_enable_mid_curve_seeds_at_current_kappa(self) -> None:
  ...
  def test_reenable_reseeds_prediction(self) -> None:
  ...
  def test_reset_reseeds_everything(self) -> None:
    s = _smoother(menu=2.0)
...
class TestElements:
  def test_entering_hysteresis_holds_in_band(self) -> None:
    s = _smoother(menu=2.0)
  ...
  def test_blend_slew_is_bounded(self) -> None:
    s = _smoother(menu=2.0)
    s.blend(0.6)
  ...
  def test_prediction_rc_scales_with_strength(self) -> None:
    outs = {}
    for menu in (1.5, 2.5):
      s = _smoother(menu=menu)
  ...
  def test_wire_hold_no_chatter_and_clean_release(self) -> None:
  ...
  def test_wire_tracks_when_stock(self) -> None:
    s = _smoother(menu=1.0)
    for i in range(20):
      v = 0.0001 * i
      assert s.wire(v) == v
