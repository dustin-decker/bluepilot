...
| **Calibration resets:** Device → Reset Camera Calibration and the model-change reset prompt clear camera calibration only. Using the Device reset after a model change now preserves learned steering delay, torque response, and vehicle parameters, avoiding an unnecessary lengthy relearn. The model prompt also preserves learned torque; it already preserved steering delay. BluePilot → Lateral Tuning (Lateral on MICI) has separate steering-delay and learned-torque reset buttons, each requiring offroad confirmation and warning that relearning takes driving time. | Dustin Decker. |
| **Settings rendering:** reopening a collapsed lateral section after viewing a setting description preserves row spacing, preventing overlapping text and controls. | Dustin Decker. |
| **Learned Stops (Observe):** BluePilot → Longitudinal Tuning has offroad mode controls and a QR link to the Portal's evidence review. Observe records stopping approaches and previews matched locations; **you control braking**. Review recorded frames, independent visits and position quality, exclude queues/maneuvers, disable approaches, and export or back up the evidence. Nearby-lead queue stops skip manual review. Exclusion buttons advance to the next pending observation while preserving history. Export/import retains verifications, approvals and review decisions. The recorder caches location/path calculations and batches input reads while retaining full-rate observations and frame indices. Sign confirmation does not bypass positioning requirements. Control is unavailable pending further simulation and physical validation; this release does not provide automatic stopping at learned locations. | Dustin Decker. |

Development checks cover added and modified Python functions across `bp-exp`, including native parameter API regressions: see [tooling setup](docs/analysis-tooling.md).
