...
class VASMDaemon:
  ...
  def _load_annotation_config(self):
    ...
    self._annotation_config = config
    self._annotation_loaded = False
    print("[VASM] No valid annotation config found; inference disabled until annotated via BluePilot Portal")
    return True
  ...
  def _update_core_affinity(self):
    if not self._affinity_set:
      set_core_affinity(V_ASM_SOLO_AFFINITY_CORES)
    ...
    while True:
      try:
        ...
        parked = (
          ...
          if self.sm.valid.get("carState", False) else False
        )
