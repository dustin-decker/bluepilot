...
def event(name: str, seconds: float, valid: bool = True, **fields: Any) -> Any:
  return SimpleNamespace(which=lambda: name, logMonoTime=int(seconds * 1e9), valid=valid,
                         **{name: SimpleNamespace(**fields)})


def test_recorded_clock_not_utc_date_selects_thursday() -> None:
...
def test_missing_clock_not_invented_and_invalid_speed_excluded() -> None:
...
def test_repeated_route_start_metadata_does_not_inflate_segment_duration() -> None:
  wall = int(datetime(2026, 9, 4, 4, tzinfo=UTC).timestamp() * 1e9)
...
def test_qlog_preferred_and_rlog_fallback(tmp_path: Path) -> None:
...
def test_ford_candidates_require_fresh_state_and_grip_cooldown() -> None:
  events = []
  for i in range(80):
    t = 1 + i / 10
  ...
  final_row = summarize(events, ZoneInfo('UTC'), ford_autocal=True)
  assert final_row is not None
  assert final_row['ford_candidate_high'] == 30
